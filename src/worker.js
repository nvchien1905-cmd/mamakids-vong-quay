// ════════════════════════════════════════════════════════════
//  MAMAKIDS VÒNG QUAY MAY MẮN — Cloudflare Workers
...
//  KiotViet API
// ════════════════════════════════════════════════════════════
const TARGET_BRANCHES = ['MMK Thọ An', 'MMK Thượng Mỗ'];
...
async function gsAppend(creds, entry) {
  try {
    ...
    const range = encodeURIComponent(`${CFG.GS_TAB}!A:F`);
    await fetch(`${GS_BASE(CFG.GS_SHEET_ID)}/values/${range}:append?valueInputOption=USER_ENTERED`, {
      method:  'POST',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
      body:    JSON.stringify({ values: [[
        entry.phone || '', entry.invoiceCode || '', entry.prizeEmoji || '',
        entry.prize || '', entry.spinTime || '', entry.spinTimeISO || '',
      ]] }),
    });
  } catch (err) {
...
async function gsRead(creds) {
  try {
    const token = await getGsToken(creds);
    const range = encodeURIComponent(`${CFG.GS_TAB}!A2:F`);
    const res = await fetch(`${GS_BASE(CFG.GS_SHEET_ID)}/values/${range}`, {
      headers: { 'Authorization': `Bearer ${token}` },
    });
    if (!res.ok) return null;
    const d = await res.json();
    return (d.values || []).map(r => ({
      phone: r[0] || '', invoiceCode: r[1] || '', prizeEmoji: r[2] || '',
      prize: r[3] || '', spinTime: r[4] || '', spinTimeISO: r[5] || '',
    }));
  } catch (err) {
...
async function gsClear(creds) {
  try {
    const token = await getGsToken(creds);
    const range = encodeURIComponent(`${CFG.GS_TAB}!A2:F`);
    await fetch(`${GS_BASE(CFG.GS_SHEET_ID)}/values/${range}:clear`, {
      method:  'POST',
      headers: { 'Authorization': `Bearer ${token}`, 'Content-Type': 'application/json' },
    });
  } catch (err) {
...
// ════════════════════════════════════════════════════════════
//  Cloudflare KV — lưu lịch sử (backup)
// ════════════════════════════════════════════════════════════
...
async function handleInvoice(url) {
  ...
  try {
    ...
    // Bước 1: Xác minh SĐT thuộc chi nhánh Mamakids
    const custData  = await kiotFetch(
      `/customers?contactNumber=${encodeURIComponent(phone)}&pageSize=5${branchQs}`
    );
    const customers = custData.data || [];
...
async function handleLoyalty(url) {
  ...
  try {
    ...
    const [custData, allGroups] = await Promise.all([
      kiotFetch(`/customers?contactNumber=${encodeURIComponent(phone)}&pageSize=5&includeCustomerGroup=true${branchQs}`),
...
export default {
  async fetch(request, env) {
    ...
    // ── Serve HTML ─────────────────────────────────────────
    if (method === 'GET' && (url.pathname === '/' || url.pathname === '/mamakids-vong-quay.html')) {
      const assetUrl = new URL('/mamakids-vong-quay.html', request.url);
    ...
    // ── /api/invoices ──────────────────────────────────────
    if (url.pathname === '/api/invoices' && method === 'GET') return handleInvoices(url);

    // ── /api/invoice ───────────────────────────────────────
    if (url.pathname === '/api/invoice' && method === 'GET') return handleInvoice(url);

    // ── /api/history ───────────────────────────────────────
    if (url.pathname === '/api/history') return handleHistory(request, env, gsCreds);

    // ── /api/loyalty ───────────────────────────────────────
    if (url.pathname === '/api/loyalty' && method === 'GET') return handleLoyalty(url);

    // ── /api/top-customers ─────────────────────────────────
    if (url.pathname === '/api/top-customers' && method === 'GET') return handleTopCustomers(env);

    // ── /api/customergroups ────────────────────────────────
    if (url.pathname === '/api/customergroups' && method === 'GET') {
      try { return jsonResp(await getCustomerGroups()); }
